#!/usr/bin/env bash
# Practical smoke test for: htrust mobile
#
# Usage shown below is executed literally.

set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
[ -x "$PROJECT_ROOT/target/release/htrust" ] || cargo build --release
export PATH="$PROJECT_ROOT/target/release:$PATH"

# Make sure these tests do not accidentally pick up environment tokens.
unset OPENAPI_TOKEN OPENAPI_SANDBOX_TOKEN

set -x

# Without a token this must fail.
if htrust mobile +393331234567 >/tmp/mobile-err 2>&1; then
  echo "FAIL: mobile should fail without a token" >&2
  exit 1
fi
grep -q OPENAPI_TOKEN /tmp/mobile-err

# Missing value must fail.
if htrust mobile >/tmp/mobile-err 2>&1; then
  echo "FAIL: mobile should fail without a value" >&2
  exit 1
fi

# Invalid format must be rejected locally, before any API call.
if htrust mobile not-a-number >/tmp/mobile-err 2>&1; then
  echo "FAIL: invalid mobile should be rejected locally" >&2
  exit 1
fi
grep -q "invalid mobile format" /tmp/mobile-err

# Live sandbox call, only if a token is available.
if [ -n "${OPENAPI_SANDBOX_TOKEN:-}" ]; then
  OPENAPI_SANDBOX_TOKEN="$OPENAPI_SANDBOX_TOKEN" htrust --sandbox mobile +393331234567
  OPENAPI_SANDBOX_TOKEN="$OPENAPI_SANDBOX_TOKEN" htrust --sandbox mobile +393331234567 --detail
  OPENAPI_SANDBOX_TOKEN="$OPENAPI_SANDBOX_TOKEN" htrust --sandbox mobile +393331234567 --json
fi
